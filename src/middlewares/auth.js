...
const userAuth = function (req, res, next) {
    try {
        ...
        if (!verify) {
            res.status(400).send({ status: false, message: `Invalid authentication token in request` })
            return;
        }
        req.userId = verify.userId; //matching userId for which token generated by the userId provided in the request.
        next()
